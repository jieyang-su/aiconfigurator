...
def _dsv4_attention_granular_ops(
    *,
    ...
    ops_list = [
        ops.GEMM(
            f"{phase}_dsv4_q_a_proj_c{compress_ratio}", scale_factor, q_lora_rank, hidden_size,
            gemm_quant_mode, seq_split=seq_split,
        ),
        ops.GEMM(
            f"{phase}_dsv4_wkv_proj_c{compress_ratio}", scale_factor, head_dim, hidden_size,
            gemm_quant_mode, seq_split=seq_split,
        ),
        ops.ElementWise(
            f"{phase}_dsv4_q_kv_norm_c{compress_ratio}", scale_factor,
            q_lora_rank + head_dim, q_lora_rank + head_dim, 0.8, seq_split=seq_split,
        ),
        ops.GEMM(
            f"{phase}_dsv4_q_b_proj_c{compress_ratio}", scale_factor,
            local_heads * head_dim, q_lora_rank, gemm_quant_mode, seq_split=seq_split,
        ),
    ]
    ...
    if compress_ratio:
        ...
        for kind in ("kv", "gate"):
            ops_list.append(
                ops.GEMM(
                    f"{phase}_dsv4_main_compressor_{kind}_c{compress_ratio}", scale_factor,
                    compressor_mult * head_dim, hidden_size, gemm_quant_mode, seq_split=seq_split,
                )
            )
        ops_list.append(
            ops.ElementWise(
                f"{phase}_dsv4_main_compress_store_c{compress_ratio}", scale_factor,
                compressor_mult * compress_ratio * head_dim,
                head_dim + common.deepseek_v4_indexer_cache_entry_bytes(head_dim),
                0.65, seq_split=seq_split,
            )
        )
    ...
    if compress_ratio == 4:
        ...
        ops_list.extend(
            [
                ops.GEMM(
                    f"{phase}_dsv4_index_q_proj", scale_factor,
                    index_n_heads * index_head_dim, q_lora_rank, gemm_quant_mode,
                    seq_split=seq_split,
                ),
                ops.GEMM(
                    f"{phase}_dsv4_index_weight_proj", scale_factor,
                    index_n_heads, hidden_size, common.GEMMQuantMode.bfloat16,
                    seq_split=seq_split,
                ),
                ops.GEMM(
                    f"{phase}_dsv4_index_compressor_kv", scale_factor,
                    2 * index_head_dim, hidden_size, gemm_quant_mode, seq_split=seq_split,
                ),
                ops.GEMM(
                    f"{phase}_dsv4_index_compressor_gate", scale_factor,
                    2 * index_head_dim, hidden_size, gemm_quant_mode, seq_split=seq_split,
                ),
                ops.ElementWise(
                    f"{phase}_dsv4_index_norm_rope_quant", scale_factor,
                ...
                ops.DSAIndexScore(
                    f"{phase}_dsv4_index_score", scale_factor, layout=layout,
                    index_heads=index_n_heads, index_head_dim=index_head_dim,
                    index_topk=index_topk, cp_size=cp_size, context_stride=4,
                ),
                ops.DSATopKSelect(
                    f"{phase}_dsv4_topk", scale_factor, layout=layout,
                    index_topk=index_topk, cp_size=cp_size, context_stride=4,
                    kernel_recipe="dsv4",
                ),
            ]
    ...
    if is_context and cp_size > 1:
        if compress_ratio == 4:
            ops_list.append(
                ops.DeepSeekV4KVAllGather(
                    f"{phase}_dsv4_index_k_all_gather", scale_factor,
                    kind="index", width=index_head_dim, cp_size=cp_size,
                )
            )
        else:
            ops_list.append(
                ops.DeepSeekV4KVAllGather(
                    f"{phase}_dsv4_window_kv_all_gather_c{compress_ratio}", scale_factor,
                    kind="window", width=head_dim, cp_size=cp_size,
        ...
        if compress_ratio:
            ops_list.append(
                ops.DeepSeekV4KVAllGather(
                    f"{phase}_dsv4_compressed_kv_all_gather_c{compress_ratio}", scale_factor,
                    kind="compressed", width=head_dim, cp_size=cp_size,
    ...
    ops_list.extend(
        [
            ops.DeepSeekV4SparseAttention(
                f"{phase}_dsv4_attention_core_c{compress_ratio}", scale_factor,
                layout=layout, local_heads=local_heads, head_dim=head_dim,
                window_size=window_size, compress_ratio=compress_ratio,
                index_topk=index_topk, kvcache_quant_mode=kvcache_quant_mode,
                fmha_quant_mode=fmha_quant_mode, cp_size=cp_size,
            ),
            ops.ElementWise(
                f"{phase}_dsv4_output_rope_c{compress_ratio}", scale_factor,
                local_heads * rope_head_dim, local_heads * rope_head_dim, 0.8,
                seq_split=seq_split,
            ),
            ...
            ops.GEMM(
                f"{phase}_dsv4_wo_a_c{compress_ratio}", scale_factor,
                local_o_groups * o_lora_rank,
                local_heads * head_dim // local_o_groups,
                common.GEMMQuantMode.bfloat16, seq_split=seq_split,
            ),
            ops.GEMM(
                f"{phase}_dsv4_wo_b_c{compress_ratio}", scale_factor,
                hidden_size, local_o_groups * o_lora_rank, gemm_quant_mode,
                seq_split=seq_split,
            ),
        ]
    )
...
def _dsv4_attention_with_granular(
    *, is_context: bool, name: str, scale_factor: float, num_heads: int,
    native_heads: int, tp_size: int, hidden_size: int, q_lora_rank: int,
    o_lora_rank: int, head_dim: int, rope_head_dim: int, index_n_heads: int,
    index_head_dim: int, index_topk: int, window_size: int, compress_ratio: int,
    o_groups: int, kvcache_quant_mode, fmha_quant_mode, gemm_quant_mode,
    cp_size: int = 1, silicon_compress_ratio: int | None = None,
):
    ...
    primary_kwargs = dict(
        num_heads=num_heads, native_heads=native_heads, tp_size=tp_size,
        hidden_size=hidden_size, q_lora_rank=q_lora_rank, o_lora_rank=o_lora_rank,
        head_dim=head_dim, rope_head_dim=rope_head_dim, index_n_heads=index_n_heads,
        index_head_dim=index_head_dim, index_topk=index_topk, window_size=window_size,
        compress_ratio=(compress_ratio if silicon_compress_ratio is None else silicon_compress_ratio),
        o_groups=o_groups,
        kvcache_quant_mode=kvcache_quant_mode, fmha_quant_mode=fmha_quant_mode,
    ...
    fallback = _dsv4_attention_granular_ops(
        phase=phase, scale_factor=scale_factor, local_heads=num_heads,
        hidden_size=hidden_size, q_lora_rank=q_lora_rank, o_lora_rank=o_lora_rank,
        head_dim=head_dim, rope_head_dim=rope_head_dim, index_n_heads=index_n_heads,
        index_head_dim=index_head_dim, index_topk=index_topk, window_size=window_size,
        compress_ratio=compress_ratio, local_o_groups=o_groups,
        kvcache_quant_mode=kvcache_quant_mode, fmha_quant_mode=fmha_quant_mode,
        gemm_quant_mode=gemm_quant_mode, cp_size=cp_size,
    )
        ...
        def _attention_ops(is_context: bool, scale_factor: float):
            ...
            return [
                _dsv4_attention_with_granular(
                    is_context=is_context, name=name, scale_factor=count * scale_factor,
                    num_heads=local_heads, native_heads=self._num_heads, tp_size=tp_size,
                    hidden_size=h, q_lora_rank=deepseek_v4_cfg.q_lora_rank,
                    o_lora_rank=deepseek_v4_cfg.o_lora_rank, head_dim=deepseek_v4_cfg.head_dim,
                    ...
                    window_size=deepseek_v4_cfg.sliding_window, compress_ratio=ratio,
                    o_groups=local_o_groups, kvcache_quant_mode=kvcache_quant_mode,
                    fmha_quant_mode=fmha_quant_mode, gemm_quant_mode=gemm_quant_mode,
