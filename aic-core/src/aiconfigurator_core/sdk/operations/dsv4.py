...
class DeepSeekV4KVAllGather(Operation):
    ...
    def __init__(
        self, name: str, scale_factor: float, *, kind: str, width: int,
        cp_size: int, window_size: int = 0, compress_ratio: int = 1,
    ) -> None:
        super().__init__(name, scale_factor)
...
class DeepSeekV4SparseAttention(Operation):
    ...
    @classmethod
    def _compressed_causal_pairs(
        cls, batch: int, query: int, prefix: int, ratio: int, limit: int
    ) -> int:
    ...
    def query(self, database: PerfDatabase, **kwargs) -> PerformanceResult:
        ...
        if self._layout == "ragged":
            ...
            if self._compress_ratio:
                ...
                pairs += self._compressed_causal_pairs(
                    batch, query, prefix, self._compress_ratio, compressed_limit
                )
        else:
            query = 1
        ...
        effective_fp8 = (
            self._kvcache_quant_mode == common.KVCacheQuantMode.fp8
            or self._fmha_quant_mode == common.FMHAQuantMode.fp8
        )
