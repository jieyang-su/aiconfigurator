...
def _replay_model_prefix(model: str) -> str:
    """Return a stable, filesystem-safe model prefix for replay bundles."""
...
def _normal_workload(
    ...
    *,
    ...
    if recv_topk_ids is None:
        if allow_missing_dispatch_fallback:
            return _router_rank_local_workload(
                record,
                rank=rank,
                layer_index=layer_index,
                num_local_physical_experts=num_local_physical_experts,
                model_layer_id=model_layer_id,
                fallback_reason="missing_recv_topk_ids",
            )
...
def materialize_replay_bundle(
    *,
    ...
    model_prefix = _replay_model_prefix(model)
    name = (
        f"{model_prefix}_{workload_source}_ep{requested_ep_size}_runtimeep{runtime_ep_size}_"
...
def materialize_synthetic_replay_bundle(
    *,
    ...
    model_prefix = _replay_model_prefix(model)
    name = (
        f"{model_prefix}_{workload_source}_ep{requested_ep_size}_runtimeep1_"
