...
def _select_wideep_latency(row: dict[str, str], source: dict[str, str] | None, phase: str) -> tuple[float, str, str]:
    ...
    if phase == "context":
        if token <= 64:
            ...
            return (
                origin * max(0.0, scale),
                "origin_latency_ep_continuous_scale",
                "wideep_context_small_origin_ep_scale",
            )
        if token <= 512:
    ...
    if source_latency is not None:
        selected = source_latency
...
def _apply_wideep_generation_capacity_envelope(
    ...
    prior_log_slopes: list[float] = []
    for token in tokens:
        row = by_token[token]
        regime = str(row.get("kernel_regime", "")).lower()
        latency = _row_latency(row)
        if latency is None or "capacity_1024" not in regime:
            continue
        if previous_token is not None and previous_latency is not None and token > previous_token:
            log_delta = _log2_token(token) - _log2_token(previous_token)
            if log_delta > 0.0:
                slope = (latency - previous_latency) / log_delta
                if slope > 0.0 and previous_token >= 128 and token <= 512:
                    prior_log_slopes.append(slope)
                if token >= 896 and prior_log_slopes:
                    floor = previous_latency + max(prior_log_slopes) * 1.15 * log_delta
                    if latency < floor:
                        _set_clean_latency(
                            row,
                            floor,
                            "wideep_generation_capacity1024_tail_slope_floor",
                        )
                        latency = floor
