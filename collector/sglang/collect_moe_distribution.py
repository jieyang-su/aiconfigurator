...
def _model_config(model_id: str) -> dict:
    """Load the model's MoE metadata without assuming a model family."""
    config_dir = _runtime_model_path(model_id)
    with open(Path(config_dir) / "config.json", encoding="utf-8") as handle:
...
def _model_moe_metadata(model_id: str) -> dict[str, int] | None:
    config = _model_config(model_id)
    num_experts = config.get("n_routed_experts", config.get("num_local_experts", config.get("num_experts")))
    # ``top_k`` is also a common generation sampling parameter (Kimi-K3 uses
    # 50 there). Prefer the model's explicit per-token expert field whenever
    # it exists; otherwise fall back to the older MoE naming variants.
    topk = config.get(
        "num_experts_per_tok",
        config.get(
            "num_experts_per_token",
            config.get("topk", config.get("top_k")),
        ),
    )
    if num_experts is None or topk is None:
        return None
    layers = config.get("num_hidden_layers", config.get("num_layers", 4))
    first_dense = config.get("first_k_dense_replace", config.get("num_dense_layers", 0))
    return {
        "num_experts": int(num_experts),
        "topk": int(topk),
        "num_layers": int(layers),
        "first_k_dense_replace": int(first_dense),
    }


def _recorded_moe_runner_backend(model_id: str, config: dict, sm_version: int) -> str:
    """Choose the runner for the real routing probe.

    The distribution probe executes the model's actual MoE layer, so it must
    follow the checkpoint's effective expert format instead of using a backend
    selected for a different synthetic case. In particular, SM90 cannot
    execute DeepGEMM's packed-FP4 W4A8 path.
    """
    override = os.environ.get("COLLECTOR_MOE_DISTRIBUTION_MOE_RUNNER_BACKEND")
    if override:
    ...
    if isinstance(nested, dict):
        ...
        effective_config.update(nested)
    else:
    ...
    try:
        from case_generator import get_sglang_moe_backend_for_model

        return get_sglang_moe_backend_for_model(model_id, moe_type, sm_version)
    except (ImportError, ValueError):
        # The recorder can run against a model not yet represented in the
        # case YAML. Let SGLang resolve that unknown model rather than falling
        # back to the unsafe historical DeepGEMM default.
        return "auto"
...
def get_moe_distribution_test_cases(model_path: str | None = None):
    selected_model = model_path or _selected_model_path()
    metadata = _model_moe_metadata(selected_model)
    if metadata is None:
        return []
...
def run_moe_distribution(
    ...
    *,
    ...
    metadata = _model_moe_metadata(model_id)
    if metadata is None:
        raise ValueError(f"Model {model_id!r} has no supported MoE expert metadata")
    num_layers = int(os.environ.get(
        "COLLECTOR_MOE_DISTRIBUTION_NUM_LAYERS",
        str(min(4, metadata["num_layers"])),
    ))
    first_k_dense_replace = int(os.environ.get(
        "COLLECTOR_MOE_DISTRIBUTION_FIRST_K_DENSE_REPLACE",
        str(metadata["first_k_dense_replace"]),
    ))
    topk = int(os.environ.get("COLLECTOR_MOE_DISTRIBUTION_TOPK", str(metadata["topk"])))
    num_experts = int(os.environ.get(
        "COLLECTOR_MOE_DISTRIBUTION_NUM_EXPERTS",
        str(metadata["num_experts"]),
    ))
    sm_major, sm_minor = torch.cuda.get_device_capability(gpu_id)
    sm_version = sm_major * 10 + sm_minor
    recorded_moe_backend = _recorded_moe_runner_backend(
        model_id,
...
# SGLang 0.5.18 sizes the detail recorder's expert-id buffer for DeepSeek-V3
# top-8.  Recorded collection is model-generic, so size it from the current
# model metadata (Kimi-K3 routes top-16) instead of truncating expert IDs.
expert_distribution_module._DetailSinglePassGatherer._TOP_K_NUM = topk
...
# Kimi-K3 is a multimodal wrapper around a text MoE backbone.  SGLang 0.5.18
# has a valid K3 model implementation, but that wrapper does not yet expose
# the small metadata hook used by ExpertDistributionRecorder.  Supply only
# that structural metadata for recording; routing and expert IDs still come
# from the actual K3 forward pass.
_original_expert_location_config = ModelConfigForExpertLocation.from_model_config

def _expert_location_config_with_kimi_k3(model_config):
    result = _original_expert_location_config(model_config)
...
ModelConfigForExpertLocation.from_model_config = staticmethod(
    _expert_location_config_with_kimi_k3
)
...
# Some MoE checkpoints wrap the language model config (and its layer-count
# fields) below ``text_config``.  Update that nested config as well so the
# small-layer collector does not accidentally construct the full multimodal
# model.  SGLang merges dict-valued overrides into PretrainedConfig objects.
try:
    raw_config_path = Path(model_path) / "config.json"
    raw_config = json.loads(raw_config_path.read_text(encoding="utf-8"))
except (FileNotFoundError, OSError, json.JSONDecodeError):
    raw_config = {}
if isinstance(raw_config.get("text_config"), dict):
    model_override["text_config"] = {
        "num_hidden_layers": num_layers,
        "first_k_dense_replace": first_k_dense_replace,
    }

is_kimi_k3 = "kimi-k3" in model_path.lower() or "kimi_k3" in model_path.lower()
recorded_moe_backend = os.environ["AIC_MOE_DISTRIBUTION_MOE_RUNNER_BACKEND"]
sm_version = os.environ["AIC_MOE_DISTRIBUTION_SM_VERSION"]
print(
    "Recorded MoE runtime selection: "
    f"model={os.environ.get('AIC_MOE_DISTRIBUTION_MODEL_ID', model_path)}, "
    f"sm={sm_version}, requested_backend={recorded_moe_backend}"
)
...
engine_kwargs = dict(
    ...
    # The recorder submits synthetic text input_ids only.  Some text-capable
    # MoE checkpoints (notably Kimi-K3) include a vision_config and would
    # otherwise enter SGLang's processor path before the tokenizer can be
    # skipped.  Keep the model config intact while disabling unused MM setup.
    ...
    # Resolve this from the loaded checkpoint and hardware. Do not force
    # DeepGEMM here: its packed-FP4 path is not valid on SM90.
    moe_runner_backend=recorded_moe_backend,
...
# SGLang starts scheduler workers with a fresh Python interpreter.  Install
# the same narrow patch as ``sitecustomize`` so those workers see it before
# ModelRunner initializes ExpertDistributionRecorder.  The directory is
# temporary and is removed with the collector subprocess workspace.
if is_kimi_k3:
    compat_dir = recorder_dir / "python_compat"
...
expert_distribution_module._DetailSinglePassGatherer._TOP_K_NUM = int(
    os.environ["AIC_MOE_DISTRIBUTION_TOPK"]
)
\n_original = ModelConfigForExpertLocation.from_model_config
\ndef _kimi_k3_expert_location(model_config):
    result = _original(model_config)
    if result is not None:
        return result
    hf_config = model_config.hf_config
    if "KimiK3ForConditionalGeneration" not in getattr(hf_config, "architectures", ()):
        return None
    text_config = getattr(hf_config, "text_config", hf_config)
    num_layers = getattr(text_config, "num_hidden_layers", None)
    num_experts = getattr(text_config, "num_experts", getattr(text_config, "n_routed_experts", None))
    if num_layers is None or num_experts is None:
        return None
    return ModelConfigForExpertLocation(int(num_layers), int(num_experts), getattr(text_config, "n_group", None))
\nModelConfigForExpertLocation.from_model_config = staticmethod(_kimi_k3_expert_location)
""",
        encoding="utf-8",
    )
    os.environ["PYTHONPATH"] = str(compat_dir) + os.pathsep + os.environ.get(
        "PYTHONPATH", ""
    )
...
try:
    ...
    # Avoid SGLang's launch-time DeepGEMM precompile sweep over every M in
    # 1..16384.  It is unrelated to this recorder case and can be killed by
    # the runtime after several minutes.  Warm up the exact target request
    # instead, so its JIT compilation and kernel selection finish before the
    # recorder is enabled.
    engine.generate(
        input_ids=input_ids,
        ...
        env.update(
            {
                ...
                "AIC_MOE_DISTRIBUTION_MOE_RUNNER_BACKEND": recorded_moe_backend,
                "AIC_MOE_DISTRIBUTION_SM_VERSION": str(sm_version),
                ...
                # The target-case generate above is the collector's warmup.
                # Do not trigger SGLang's unbounded launch-time precompile.
