...
def collect_sglang(
    ...
    # Recorded MoE cases depend on the distribution sidecar generated by
    # ``moe_token_distribution``.  Case generation happens in the parent
    # process before workers start, so running both entries in one list would
    # discover the MoE cases too early.  Run the sidecar phase first, then
    # build the remaining cases with the completed file available.
    distribution_collections = [
        collection for collection in pool_collections if collection["type"] == "moe_token_distribution"
    ]
    if distribution_collections:
        ...
        distribution_file = Path.cwd() / str(distribution_collections[0]["perf_filename"])
        if not distribution_file.is_file():
            raise RuntimeError(
                "moe_token_distribution completed without producing "
                f"the Recorded sidecar: {distribution_file}"
            )
        os.environ["COLLECTOR_MOE_RECORDED_DISTRIBUTION_FILE"] = str(distribution_file)
        os.environ.setdefault("COLLECTOR_MOE_RECORDED_DISTRIBUTION", "recorded")
        logger.info(f"Recorded MoE sidecar ready: {distribution_file}")
        pool_collections = [
            collection for collection in pool_collections if collection["type"] != "moe_token_distribution"
        ]
