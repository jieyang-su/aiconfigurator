...
def test_v4_ratio_specific_granular_graphs():
    ...
    for phase in ("context", "generation"):
        ...
        assert all(
            isinstance(op, FallbackOp)
            and common.DatabaseMode.ANALYTICAL in op._primary_excluded_modes
            for op in wrappers
        )
        ...
        assert len([op for op in by_ratio[4] if isinstance(op, GEMM)]) > len(
            [op for op in by_ratio[128] if isinstance(op, GEMM)]
...
def test_v4_context_cp_granular_restores_ratio_specific_all_gathers():
    ...
    assert [op._kind for op in by_ratio[4] if isinstance(op, DeepSeekV4KVAllGather)] == [
        "index", "compressed"
    ]
    assert [op._kind for op in by_ratio[128] if isinstance(op, DeepSeekV4KVAllGather)] == [
        "window", "compressed"
    ]
...
def test_v4_mhc_analytical_is_no_table():
    ...
    database = get_database_view(
        "h100_sxm", "sglang", "estimate", allow_missing_data=True,
...
def test_v4_fp8_static_analytical_runs_end_to_end_without_module_tables():
    model = get_model(
        "sgl-project/DeepSeek-V4-Flash-FP8",
        config.ModelConfig(
            tp_size=1, moe_tp_size=1, moe_ep_size=1, nextn=1, overwrite_num_layers=2,
        ),
        backend_name="sglang",
    )
    database = get_database_view(
