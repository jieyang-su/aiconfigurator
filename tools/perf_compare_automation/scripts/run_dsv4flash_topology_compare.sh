...
SCALEUP_SYSTEM="${SCALEUP_SYSTEM:-rtx_pro_6000_scaleup_32x1}"
SCALEOUT_SYSTEM="${SCALEOUT_SYSTEM:-rtx_pro_6000_scaleout_8x4}"
mkdir -p "$OUT_DIR"
...
start_runs() {
  : > "$CMD_LOG"
  : > "$PID_FILE"
  run_case "$SCALEUP_SYSTEM" "$OUT_DIR/output_scaleup.log" "SCALEUP_PID" "$OUT_DIR/scaleup"
  run_case "$SCALEOUT_SYSTEM" "$OUT_DIR/output_scaleout.log" "SCALEOUT_PID" "$OUT_DIR/scaleout"
